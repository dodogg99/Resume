## VBA-project
 #### VBA-project為我在公司進行問題處理時撰寫的VBA程式，因此無法呈現資料執行完的結果。下列分別針對5個問題進行說明，並列出這些程式主要使用的VBA語法，最後以存貨呆滯分析為範例簡述程式步驟及完整程式碼。

## 檔案說明
* ### 刀具使用分析:
  #### 問題 : 從機台更換刀具的數據中找出異常使用狀況
  #### 方法 : 使用 $\mathbb{\color{red}{樞紐分析表}}$ 分析刀具更換紀錄，從換刀原因、加工產品、刀具類型等不同層面進行分析。
  #### 程式減少人力時間 : 30分鐘
* ### 刀具呆滯分析: 
  #### 問題 : 刀具庫存過高，須了解刀具使用狀況以進行改善對策
  #### 方法 : 將刀具庫存及領用紀錄進行分析，找出呆滯刀具及曾經加工的產品並統計總庫存金額、呆滯刀具金額及本月領用金額。
  #### 程式減少人力時間 : 60分鐘
  - 引用VBAProject : Microsoft Scripting Runtime
* ### 即時生產機況:
  #### 問題 : 機聯網看板只以機台號碼來顯示生產產品，想了解各個產品的生產情況只能自行統計
  #### 方法 : 使用 $\color{red}{QueryTable}$下載網頁機台生產資料，整理成各個產品在哪些機台生產、生產機台總數的即時訊息。
  #### 程式減少人力時間 : 30分鐘
  - 引用VBAProject : Microsoft Scripting Runtime
* ### 存貨呆滯分析:
  #### 問題 : 分析部門每月提列存貨呆滯金額變動
  #### 方法 : 將兩個月份的存貨進行 $\mathbb{\color{red}{篩選}}$比較，找出異常增加的存貨項目及原因。
  #### 程式減少人力時間 : 30分鐘
  - 引用VBAProject : Microsoft Scripting Runtime
* ### 績效數據下載及整理:
  #### 問題 : 部門每月生產績效指標更新
  #### 方法 : 使用 $\color{red}{Selenium}$控制chrome瀏覽器下載資料庫網頁API中的績效數據，更新每月的績效指標。
  #### 程式減少人力時間 : 30分鐘
